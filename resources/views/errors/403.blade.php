...
<head>
  ...
  <title>MotoTrack — 403 Akses Ditolak</title>
  ...
  <style>
    ...
    .lock-shake    { animation: lock-shake 1.2s ease-in-out infinite; animation-delay: 2s; }
    ...
    .stat-chip:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(220,38,38,.12); }

    .btn-primary { transition: transform .15s, box-shadow .15s, background .15s; }
    .btn-primary:hover { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(220,38,38,.28); }
...
<body class="font-sans bg-slate-50 text-slate-900 min-h-screen overflow-x-hidden">
  ...
  <div class="fixed top-0 right-0 w-[500px] h-[500px] bg-red-100/40 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
  <div class="fixed bottom-0 left-0 w-[400px] h-[400px] bg-rose-100/30 rounded-full blur-3xl translate-y-1/2 -translate-x-1/3 pointer-events-none"></div>
  ...
  <header class="relative z-10 bg-white/80 backdrop-blur-md border-b border-slate-200 px-5 md:px-8 h-[56px] flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      <div class="hidden sm:flex items-center gap-1.5 bg-red-50 border border-red-200 px-2.5 py-1 rounded-lg">
        ...
        <span class="text-[10px] font-bold text-red-500">ERROR 403</span>
      </div>
    ...
    <div class="w-full max-w-2xl mx-auto">
      ...
      <div class="flex justify-center mb-6 fade-up" style="animation-delay:.05s">
        <div class="inline-flex items-center gap-2 bg-red-50 border border-red-200 px-4 py-1.5 rounded-full">
          <i data-lucide="shield-x" style="width:13px;height:13px" class="text-red-500"></i>
          <span class="text-[11px] font-bold text-red-500 uppercase tracking-widest">Akses Ditolak</span>
        </div>
      </div>
        ...
        <div class="relative w-[220px] h-[220px] md:w-[260px] md:h-[260px] flex items-center justify-center">
          ...
          <div class="absolute inset-0 flex items-center justify-center spin-slow">
            <svg width="200" height="200" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="100" cy="100" r="88" stroke="#fecaca" stroke-width="1.5" stroke-dasharray="6 6"/>
            </svg>
          </div>
          <div class="relative w-[110px] h-[110px] md:w-[126px] md:h-[126px] bg-white rounded-full border-2 border-slate-200 shadow-xl flex items-center justify-center">
            <div class="float-icon lock-shake">
              <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                <!-- Lock body -->
                <rect x="10" y="28" width="40" height="28" rx="6" fill="#fee2e2" stroke="#dc2626" stroke-width="2.5"/>
                <!-- Lock shackle -->
                <path d="M18 28V20a12 12 0 0 1 24 0v8" stroke="#dc2626" stroke-width="2.5" stroke-linecap="round" fill="none"/>
                <!-- Keyhole -->
                <circle cx="30" cy="41" r="4" fill="#dc2626"/>
                <rect x="28" y="42" width="4" height="7" rx="1" fill="#dc2626"/>
                <!-- X mark -->
                <line x1="23" y1="8" x2="29" y2="14" stroke="#ef4444" stroke-width="2" stroke-linecap="round"/>
                <line x1="29" y1="8" x2="23" y2="14" stroke="#ef4444" stroke-width="2" stroke-linecap="round"/>
              </svg>
            </div>
          </div>
          <div class="absolute top-3 right-8 w-2.5 h-2.5 bg-red-400 rounded-full signal-ripple opacity-70"></div>
          <div class="absolute top-3 right-8 w-2.5 h-2.5 bg-red-400 rounded-full signal-ripple"></div>
          <div class="absolute bottom-6 left-6 w-2 h-2 bg-rose-400 rounded-full signal-ripple opacity-60" style="animation-delay:.9s"></div>
      ...
      <div class="text-center mb-5">
        <div class="error-code font-black text-[110px] md:text-[130px] leading-none text-transparent bg-clip-text bg-gradient-to-b from-red-600 to-red-300 fade-up select-none" style="animation-delay:.15s">
          403
        </div>
        ...
        <p class="text-slate-500 text-[13px] md:text-[14px] font-medium max-w-sm mx-auto leading-relaxed fade-up" style="animation-delay:.25s">
          Perangkat GPS tidak memiliki izin — kamu tidak berwenang mengakses area terbatas ini dalam sistem MotoTrack.
        </p>
      </div>
      ...
      <div class="bg-slate-900 rounded-2xl px-5 py-4 mb-5 mx-auto max-w-lg fade-up" style="animation-delay:.3s">
        ...
        <div class="font-mono text-[11px] md:text-[12px] flex flex-col gap-1.5" id="terminalLines">
          <div class="terminal-line text-slate-400"><span class="text-emerald-400 font-bold">$</span> gps --auth <span class="text-blue-400">--url="{{ request()->path() ?? 'halaman-ini' }}"</span></div>
          <div class="terminal-line text-yellow-400"><span class="text-slate-500">›</span> Memverifikasi kredensial pengguna...</div>
          <div class="terminal-line text-red-400"><span class="text-slate-500">›</span> AKSES DITOLAK: Izin tidak mencukupi</div>
          <div class="terminal-line text-red-400"><span class="text-slate-500">›</span> HTTP 403 — Forbidden. Token otorisasi tidak valid</div>
          <div class="terminal-line text-slate-400"><span class="text-slate-500">›</span> Saran: Login dengan akun yang memiliki akses (<span class="text-blue-400">dashboard</span>)</div>
          <div class="terminal-line text-amber-400"><span class="text-slate-500">›</span> Insiden ini telah <span class="font-bold text-white">dicatat</span> dalam log sistem</div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap justify-center gap-2.5 mb-7 fade-up" style="animation-delay:.35s">
        <div class="stat-chip flex items-center gap-2 bg-white border border-slate-200 px-3.5 py-2 rounded-xl shadow-sm cursor-default">
          <div class="w-6 h-6 rounded-lg bg-red-50 flex items-center justify-center">
            <i data-lucide="shield-x" style="width:12px;height:12px" class="text-red-400"></i>
          </div>
          <div>
            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Status</div>
            <div class="text-[11px] font-extrabold text-red-500">Diblokir</div>
          </div>
        </div>
        <div class="stat-chip flex items-center gap-2 bg-white border border-slate-200 px-3.5 py-2 rounded-xl shadow-sm cursor-default">
          ...
          <div>
            ...
            <div class="text-[11px] font-extrabold text-amber-500">HTTP 403</div>
          </div>
        </div>
        <div class="stat-chip flex items-center gap-2 bg-white border border-slate-200 px-3.5 py-2 rounded-xl shadow-sm cursor-default">
          <div class="w-6 h-6 rounded-lg bg-red-50 flex items-center justify-center">
            <i data-lucide="lock" style="width:12px;height:12px" class="text-red-400"></i>
          </div>
          <div>
            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Otorisasi</div>
            <div class="text-[11px] font-extrabold text-red-500">Tidak Valid</div>
          </div>
        </div>
        <div class="stat-chip flex items-center gap-2 bg-white border border-slate-200 px-3.5 py-2 rounded-xl shadow-sm cursor-default">
          <div class="w-6 h-6 rounded-lg bg-slate-100 flex items-center justify-center">
            <i data-lucide="user-x" style="width:12px;height:12px" class="text-slate-400"></i>
          </div>
          <div>
            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Pengguna</div>
            <div class="text-[11px] font-extrabold text-slate-600">Tidak Diizinkan</div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 justify-center fade-up" style="animation-delay:.42s">
        <a href="{{ route('dashboard') }}"
          class="btn-primary flex items-center justify-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white text-[13px] font-bold rounded-xl shadow shadow-red-200 no-underline">
