...
// ── Client dibuat SEKALI, dipakai terus ──
...
void pushGps() {
  ...
  http.setReuse(true);  // keep-alive: SSL handshake tidak diulang

  // if (!http.begin(secureClient, String(SERVER_URL) + "/api/device/gps")) return;
  if (!http.begin(client, String(SERVER_URL) + "/api/device/gps")) return;
...
void pollCommand() {
  ...
  // if (!http.begin(secureClient, String(SERVER_URL) + "/api/device/command")) return;
  if (!http.begin(client, String(SERVER_URL) + "/api/device/command")) return;
...
void checkWifi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Reset secure client setelah reconnect
    // secureClient.stop();
...
void setup() {
  ...
  // Setup secure client sekali saja
  // secureClient.setInsecure();
